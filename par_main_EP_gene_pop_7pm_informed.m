...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = par_main_EP_gene_pop_7pm_informed(Irmax,nD,jd,irand)
format longE;
%% Input
Irmax = 1;% 1e4;  % # of iteration in mu
nD    = 1;    % Number of the output data
jd    = 0;    % Choice searching strategy 1) jd=0 Random search; 2) jd=1 informed search
irand = 0;    % Total random irand = 1 ou random outside the region sampled irand = 0
...
for ir=1:Irmax
    ...
    parfor (i = 0:Nmax)
        ...
        DD = [DD,D]; Pm = [Pm,pm];
        ll = LogL_real_7pm_informed(mu,D,pm,jd,ic);
        lL = [lL,ll];
        %     i=i+1;
    end
...
%     toc
    ...
    if (mod(ir,50)==0)
        save(['par_main_EP_gene_7pm_pop_real_informed_jd',num2str(jd),'_L_',num2str(nD),'_rand',num2str(irand)],'MU','DDD','PPm','llL','Dmin','Dmax','Irmax')
    end   
end
% toc
% end
